  ...
  var checkBpus = function(callback) {
    ...
    var checkBpu = function(checkBpuCallback) {
      ...
      bpuObj.checkMsgs.push({
        ...
        msg: 'SocketNull?' + (bpuObj.socket === null) + ' timeoutCounter:' + bpuObj.socketTimeouts
      });
        ...
        if (!didCallback) {
          if (err) {
            ...
            bpuObj.checkMsgs.push({
              isErr: true,
              time: new Date().getTime(),
              msg: '!bpuObj.socket.connected'
            });
            app.runData.addExpToBpuErrors.push({
              time: new Date(),
              err: bpuObj.doc.name + ' fn_connectBpu ' + '!bpuObj.socket.connected'
            });
            return checkBpuCallback(null);
          } else {
            ...
            bpuObj.socket.emit(app.mainConfig.socketStrs.bpu_getStatus, function(resObj) {
              if (!didCallback) {
                ...
                bpuObj.checkMsgs.push({
                  isErr: false,
                  time: new Date().getTime(),
                  msg: 'Status=' + bpuObj.doc.bpuStatus
                });
                ...
                fn_clearBpuExp(bpuObj, function(err) {
                  ...
                  statMsg.forEach(function(stat) {
                    bpuObj.checkMsgs.push({
                      ...
                      msg: stat.msg + Math.round(stat.age / 1000) + ' secs ago'
                    });
                  });
