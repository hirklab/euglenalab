...
sudo iptables -t nat -Z; # zero counters
sudo iptables -t nat -F; # flush (delete) rules
sudo iptables -t nat -X; # delete all extra chains
...
sudo iptables -t nat -I OUTPUT -p tcp --dport $WEB_PUBLIC_PORT -j REDIRECT --to-ports $WEB_LOCAL_PORT

sudo iptables -t nat -A PREROUTING -p tcp --dport $ADMIN_PUBLIC_PORT -j REDIRECT --to-port $ADMIN_LOCAL_PORT
sudo iptables -t nat -I OUTPUT -p tcp --dport $ADMIN_PUBLIC_PORT -j REDIRECT --to-ports $ADMIN_LOCAL_PORT

sudo iptables -t nat -A PREROUTING -p tcp --dport $CONTROLLER_PUBLIC_PORT -j REDIRECT --to-port $CONTROLLER_LOCAL_PORT
sudo iptables -t nat -I OUTPUT -p tcp --dport $CONTROLLER_PUBLIC_PORT -j REDIRECT --to-ports $CONTROLLER_LOCAL_PORT
...
do
    ...
    sudo iptables -t nat -I OUTPUT -p tcp --dport $((CAMERA_PUBLIC_PORT+5*bpu)) -j DNAT --to 192.168.1.$((bpu + 200)):$CAMERA_LOCAL_PORT
done
