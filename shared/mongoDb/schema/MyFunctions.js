...
exports = module.exports = function (app) {
    ...
    //My Functions
    schema.statics.runActionOnArrayElements = function (action, array, mainCallback) {
        var functionsArray = [];
    ...
    schema.statics.runBashCommand = function (cmdStr, mainCallback) {
        var child = exec(cmdStr, function (error, stdout, stderr) {
            if (error !== null) {
                mainCallback('error: ' + stderr, stdout);
            } else if (stderr) {
                mainCallback('stderr: ' + stderr, stdout);
            } else if (stdout) {
                mainCallback(null, stdout);
            } else {
                mainCallback(null, null);
            }
        });
    };
    schema.statics.searchBackForFilename = function (filename, startPath, stopPath, mainCallback) {
    ...
    schema.statics.isLeapYear = function (year) {
        return new Date(year, 1, 29).getMonth() == 1;
    };
    schema.statics.getMilliSecToTodayHourMin = function (hour, min) {
    ...
    schema.statics.assertMyObject = function (objToTest, xProps) {
        var catchErr = null;
        try {
