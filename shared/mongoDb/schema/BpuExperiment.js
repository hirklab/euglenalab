...
exports = module.exports = function (app, mongoose) {
    var mySchema = new mongoose.Schema({
        ...
        bc_startSendTime: {
            type: Date,
            default: null
        }, //set in bpu controller when exp is popped and ready to send to exp creation when mongo creates it
        bc_isLiveSendingToLab: {
            type: Boolean,
            default: false
        },
        liveBpu: {
        ...
        exp_status: {
            type: String,
            enum: ['created', 'submited', 'queued', 'addingtobpu', 'running', 'servercleared', 'processing', 'failed', 'finished'],
            default: 'created'
        },
        ...
        }, //set on creation when mongo creates it
        exp_submissionTime: {
            type: Number,
            default: null
        }, //set on main server when it joins the queue
        exp_resortTime: {
            type: Number,
            default: null
        }, //set on main server last time the queue resorted

        exp_runStartTime: {
            type: Number,
            default: null
        }, //set on bpu when experiment starts
        exp_runEndTime: {
            type: Number,
            default: null
        }, //set on bpu when experiment ends

        exp_serverClearTime: {
            type: Number,
            default: null
        }, //set on proc server when experiment has been cleared for processing by the server
        exp_processingStartTime: {
            type: Number,
            default: null
        }, //set on proc server when experiment starts processing
        exp_processingScripterStartTime: {
            type: Number,
            default: null
        }, //set on proc server when experiment stops processing
        ...
        proc_doNotProcess: {
            type: Boolean,
            default: false
        }, //proc set when we don't want to process
        proc_attempts: {
            type: Number,
            default: 0
        }, //stat attempts processing
        proc_err: {
            type: String,
            default: null
        }, //last err for processing

        proc_startPath: {
            type: String,
            default: null
        }, //set on proc server when exp id was matched in mount data dir.

        proc_jpgFiles: {
            type: Array,
            default: []
        }, //set on proc server when validating exp data pacakge prior to starting processing

        proc_lightDataArrayPath: {
            type: String,
            default: null
        }, //set on proc server when validating exp data location of specical file

        proc_expSchemaJsonPath: {
            type: String,
            default: null
        }, //set on proc server when validating exp data location of specical file
        ...
        proc_endPath: {
            type: String,
            default: null
        }, //set on proc server when processing has finished and the tar file is made.

        user_tarFilePath: {
            type: String,
            default: null
        }, //set on clinet server when user attempts first download.
        user_tarFilename: {
            type: String,
            default: null
        }, //set on clinet server when user attempts first download.
        ...
        //Not sure if used
        exp_liveSocketHandle: {
            type: String,
            default: null
        },
        exp_liveSocketWaitingForUserHandle: {
            type: Boolean,
            default: false
        },
...
var _getDataObjToSetLeds = function () {
    var setLedsData = {
        ...
        metaData: {
            ...
            offsetX: null,
            offsetY: null, //used for all events, light value set events are converted to xy

            className: null,
            clientX: null,
            clientY: null, //only set for browser events
