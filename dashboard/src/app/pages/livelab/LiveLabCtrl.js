(function() {
	...
	angular.module('BioLab.pages.livelab')
		.controller('LiveLabCtrl', LiveLabCtrl);

	/** @ngInject */
	function LiveLabCtrl($scope, $rootScope, $http, $timeout, $stateParams, lodash, Microscope, Experiment, uiTourService, socket) {
		...
		vm.magnification = 20 * 10; // 20 is actual magnification  10 is multiplier
		...
		Experiment.detail($stateParams.id).then(function(res) {
			...
			vm.getMicroscope(res.data.bpu);
		});

		vm.getMicroscope = function(microscopeId) {
			Microscope.detail(microscopeId).then(function(res) {
				var microscope = res.data.results;
				...
				if (microscope.isActive) {
					microscope.address = 'http://' + microscope.publicAddr.ip + ':' + microscope.publicAddr.webcamPort + '?action=stream';
				} else {
				...
				vm.microscope = microscope;

				vm.magnification = microscope.magnification * 10;
					...
					if (bpuUpdates && users) {
						...
						$scope.$apply(function() {
							...
							var bpu = lodash.find(bpuUpdates, function(bpu) {
								return bpu.id === vm.microscope.id;
							});

							if (bpu != null) {
								vm.microscope.status = Microscope.BPU_STATUS_DISPLAY[bpu.bpuStatus];

								vm.microscope.allowedGroups = bpu.allowedGroups;
								vm.microscope.processingTimePerExperiment = bpu.bpu_processingTime;
								vm.microscope.timePending = (bpu.timePending > 0 ? bpu.timePending : 0);
		...
		vm.onMove = function(response) {
			...
			var radians = response.angle * Math.PI / 180;

			var horizontal = response.magnitude * Math.cos(radians);
			if (horizontal < 0) {
				vm.leds.left = Math.abs(horizontal);
				vm.leds.right = 0;
			} else {
				vm.leds.right = Math.abs(horizontal);
				vm.leds.left = 0;
			}


			var vertical = response.magnitude * Math.sin(radians);
			if (vertical < 0) {
				vm.leds.bottom = Math.abs(vertical);
				vm.leds.top = 0;
			} else {
				vm.leds.top = Math.abs(vertical);
				vm.leds.bottom = 0;
			}
