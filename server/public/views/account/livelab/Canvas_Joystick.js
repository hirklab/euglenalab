var Canvas_Joystick = function(canvasDiv) {
  ...
  thisJoy.canvas.height = canvasDiv.clientWidth * canvasPCT;
  ...
  //OriginalValues
  thisJoy.org = {};
  thisJoy.org.textIntensity = {
    x: 8,
    y: 44,
    size: '16'
  };
  thisJoy.org.textAngle = {
    x: 8,
    y: 20,
    size: '16'
  };
  ...
  thisJoy.centerPoint = {
    x: thisJoy.canvas.width * 0.500,
    y: thisJoy.canvas.height * 0.550
  };
  ...
  var drawIntensityText = function(alpha) {
    ...
    context.font = thisJoy.textIntensity.size + 'px Arial';
    ...
    context.fillText('Intensity: ' + Math.round(alpha * 100) + '%', thisJoy.textIntensity.x, thisJoy.textIntensity.y);
  };

  var drawAngleText = function(angle) {
    context.beginPath();
    context.fillStyle = "rgba(255, 255, 255, " + 1.0 + ")";
    context.font = thisJoy.textAngle.size + 'px Arial';
    ...
    context.fillText('Angle: ' + txt + '°', thisJoy.textAngle.x, thisJoy.textAngle.y);
  };
  ...
  var drawJoystickTitle = function() {
    ...
    context.fillText(title, thisJoy.canvas.width - m.width - 5, thisJoy.canvas.height - 5);
  };
...
Canvas_Joystick.prototype.resize = function(width, height) {
  ...
  if (this.canvas.width < 200) {
    ...
    this.textIntensity.size = '' + (this.canvas.width * 12 / 200);
    this.textIntensity.x = 4;
    this.textIntensity.y = 25;
