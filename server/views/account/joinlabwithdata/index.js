    ...
    query.exec(function(err, docs) {
      if (err) {
        ...
        outcome.bpus.forEach(function(bpu) {
          ...
          var activity = JSON.parse(JSON.stringify(bpu.performanceScores)).activity;
          var activityScore = activity > 50 && activity < 150 ? 5 : 0;
          var response = JSON.parse(JSON.stringify(bpu.performanceScores)).response;
...
exports.update = function(req, res, next) {
  ...
  workflow.on('patchBpuExperiment', function(user) {
    ...
    if (req.body.survey) {
      fieldsToSet['survey'] = req.body.survey;
    }
    ...
    req.app.db.models.BpuExperiment.findByIdAndUpdate(req.body._id, fieldsToSet, function(err, bpuExperiment) {
      ...
      // workflow.outcome.experiment = bpuExperiment;
