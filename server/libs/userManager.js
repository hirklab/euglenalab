...
UserManager.prototype.connect = function(controller, cb) {
	...
	that.io.sockets.on(EVENTS.CONNECT, function(socket) {
		if (socket && socket.request.session.hasOwnProperty('passport') && socket.request.session.passport && socket.request.session.passport.hasOwnProperty('user') && socket.request.session.passport.user) {
