...
var _serverHandler = function (req, res) {
	_myPrint('server_socket', 'web serverHandler NO CODE!');
};
			...
			if (exp.isLive) {
				...
				_decoder.decode(exp.data, function (err, specArr) {
					if (err) {
						...
						if (_bpuScript.exp.data.length > 1) {
							_bpuScript.exp.data.sort(function (a, b) {
						...
						_bpuScript.exp.data.forEach(function (item) {
							_bpuScript.eventsToRun.push(item);
						});
						_bpuScript.eventsToRun.sort(function (a, b) {
							return a.time - b.time;
						});
