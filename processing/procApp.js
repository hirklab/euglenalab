		...
		if (!isLoopRunning) {
			if (app.runps.doRunOneLoop && loopCounter > 0) {
				...
				if (totalTime > app.runps.loopCallbackInterval) {
					...
					lastTime = new Date();
				}     //totalTime>app.runps.loopCallbackInterval
...
var init_getBpuExpClearedByServer = function (callback) {
	...
	var queryObj        = {
		_id:               {
			$gt: generalFuncs.getMongoObjectIdFromTimestamp(greaterThanDate),
			$lt: generalFuncs.getMongoObjectIdFromTimestamp(lessThanDate)
		},
...
var p_checkScripter      = function (callback) {
	...
	var processScripter = function (cb_fn) {
		var expDoc = this;
		//Get Bpu
		app.db.models.Bpu.findOne({name: expDoc.exp_lastResort.bpuName}, {
			name:                  1,
			magnification:         1,
			performanceScores:     1,
